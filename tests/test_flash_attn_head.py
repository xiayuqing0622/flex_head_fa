...
@pytest.mark.parametrize("h,h_k,h_v",[
    (32,4,16),
])
...
def test_flash_attn_output(
    seqlen_q, seqlen_k, h, h_k, h_v, d, dropout_p, causal, local, alibi, deterministic, dtype, kvpacked, softcap
):
    if (
    ...
    batch_size = 1 # 4
    nheads = h # 6 if softcap == 0.0 else 4  # softcap reference impl takes more memory
    nheads_k = h_k # nheads if mha_type == "mha" else (1 if mha_type == "mqa" else 2)
    nheads_v = h_v
    assert nheads % nheads_k == 0
    assert nheads % nheads_v == 0
    assert (not kvpacked) or (nheads_k == nheads_v)
    ...
    if dropout_p > 0.0:
        ...
        if kvpacked:
            ...
            v_rep = repeat(v, "b s h d -> b s (h g) d", g=nheads // nheads_v)
...
def test_flash_attn_splitkv(
    seqlen_q, seqlen_k,h,h_k,h_v, swap_sq_sk, d, causal, local, alibi, deterministic, dtype
):
    if swap_sq_sk:
    ...
    v = torch.randn(batch_size, seqlen_k, nheads_v, d, device=device, dtype=dtype, requires_grad=True)
    if alibi:
        alibi_slopes = torch.rand(batch_size, nheads, device=device, dtype=torch.float32) * 0.3
        attn_bias = attn_bias_from_alibi_slopes(alibi_slopes, seqlen_q, seqlen_k, causal=causal)
    else:
