...
def _fwd_kernel(
    ...
    off_b = off_hb // nheads
    off_h = off_hb % nheads
    # head_stride = nheads // sparse_ratio
    # intercept = off_h // head_stride
    ...
    end_n = seqlen_k if not IS_CAUSAL else tl.minimum(((start_m + 1) * BLOCK_M) // sparse_ratio, seqlen_k)
    # end_n = seqlen_k if not IS_CAUSAL else tl.minimum(((start_m + 1) * BLOCK_M) // sparse_ratio - intercept, seqlen_k)
    for start_n in range(0, end_n, BLOCK_N):
        ...
        # triton bug when intercept != 0
        # if IS_CAUSAL: 
        #     qk += tl.where(offs_m[:, None] >= ((start_n + offs_n) * sparse_ratio + intercept)[None, :], 0, float("-inf"))
        if BIAS_TYPE != 'none':
            if BIAS_TYPE == 'vector':
                if EVEN_N:
...
def _bwd_kernel_one_col_block(
    ...
    begin_m = 0 if not IS_CAUSAL else (((start_n * BLOCK_N) * sparse_ratio)// BLOCK_M) * BLOCK_M
    # triton bug when intercept != 0
    # begin_m = 0 if not IS_CAUSAL else (((start_n * BLOCK_N) * sparse_ratio + intercept)// BLOCK_M) * BLOCK_M
    ...
    for start_m in range(begin_m, num_block_m * BLOCK_M, BLOCK_M):
        ...
        # triton bug when intercept != 0
        # if IS_CAUSAL: 
        #     qk = tl.where(offs_m_curr[:, None] >= ((offs_n * sparse_ratio + intercept)[None, :]), qk, float("-inf"))
        if BIAS_TYPE != 'none':
...
def _flash_attn_forward(q, k, v, bias=None, causal=False, softmax_scale=None, sparse_ratio=1):
    # shape constraints
...
def _flash_attn_backward(do, q, k, v, o, lse, dq, dk, dv, bias=None, causal=False, softmax_scale=None, sparse_ratio=1):
...
class FlashAttnFunc(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, q, k, v, bias=None, causal=False, softmax_scale=None, sparse_ratio=1):
        """
        ...
        o, lse, ctx.softmax_scale = _flash_attn_forward(
            q, k, v, bias=bias, causal=causal, softmax_scale=softmax_scale, sparse_ratio=sparse_ratio,
        )
...
flash_attn_sparse_func = FlashAttnFunc.apply