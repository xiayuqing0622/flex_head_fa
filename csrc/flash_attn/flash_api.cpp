...
void set_params_fprop_bias(Flash_fwd_params &params,
    // device pointers
    void* attn_bias,
...
// All stride are in elements, not bytes.
// params.attn_bias_row_stride = attn_bias.stride(-3);
...
//     // if (seqlenq_ngroups_swapped) {
//     // params.attn_bias_batch_stride *= seqlen_q;
//     // }
// }
...
std::vector<at::Tensor>
mha_fwd_kvcache_and_bias(at::Tensor &q,                 // batch_size x seqlen_q x num_heads x head_size
                const at::Tensor &kcache,            // batch_size_c x seqlen_k x num_heads_k x head_size or num_blocks x page_block_size x num_heads_k x head_size if there's a block_table.
                const at::Tensor &vcache,            // batch_size_c x seqlen_k x num_heads_k x head_size or num_blocks x page_block_size x num_heads_k x head_size if there's a block_table.
                const at::Tensor &attn_bias,         // batch_size x num_heads x seqlen_q x seqlen_k
                std::optional<const at::Tensor> &k_, // batch_size x seqlen_knew x num_heads_k x head_size
                std::optional<const at::Tensor> &v_, // batch_size x seqlen_knew x num_heads_k x head_size
                std::optional<const at::Tensor> &seqlens_k_, // batch_size
                std::optional<const at::Tensor> &rotary_cos_, // seqlen_ro x (rotary_dim / 2)
    ...
    if (window_size_left >= seqlen_k) { window_size_left = -1; }
    if (window_size_right >= seqlen_k) { window_size_right = -1; }
    TORCH_CHECK(!paged_KV, "Paged KV cache does not support attention bias");

    CHECK_SHAPE(q, batch_size, seqlen_q, num_heads, head_size_og);
    if (!paged_KV) {
        CHECK_SHAPE(kcache, batch_size_c, seqlen_k, num_heads_k, head_size_og);
        CHECK_SHAPE(vcache, batch_size_c, seqlen_k, num_heads_k, head_size_og);
        CHECK_SHAPE(attn_bias, batch_size_c, num_heads_k, seqlen_q, seqlen_k);
    } else {
        CHECK_SHAPE(kcache, num_blocks, page_block_size, num_heads_k, head_size_og);
        CHECK_SHAPE(vcache, num_blocks, page_block_size, num_heads_k, head_size_og);
        CHECK_SHAPE(block_table, batch_size, max_num_blocks_per_seq);
    ...
    uint32_t attn_bias_batch_stride = attn_bias.stride(0);
    uint32_t attn_bias_head_stride = attn_bias.stride(1);
    uint32_t attn_bias_q_stride = attn_bias.stride(2);
    set_params_fprop_bias(params, attn_bias.data_ptr(), attn_bias_batch_stride, attn_bias_head_stride, attn_bias_q_stride);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("fwd_kvcache_and_bias", &FLASH_NAMESPACE::mha_fwd_kvcache_and_bias, "Forward pass, with KV-cache and bias");
}
