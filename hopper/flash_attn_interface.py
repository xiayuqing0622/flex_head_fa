...
import flex_head_fa_2_cuda as flash_attn_cuda
...
def _flash_attn2_varlen_backward(
    dout,
    q,
    k,
    v,
...
class FlashAttnF3B2Func(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        q,
...
class FlashAttnF3B2VarlenFunc(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        q,
...
def flash_attn_f3b2_func(
    ...
    Return:
        ...
        S_dmask [optional, if return_attn_probs=True]: (batch_size, nheads, seqlen, seqlen).
            ...
            pattern (negative means that location was dropped, nonnegative means it was kept).
    """
    return FlashAttnF3B2Func.apply(
        q,
        k,
        v,
        softmax_scale,
...
def flash_attn_f3b2_varlen_func(
    q,
    k,
    v,
    cu_seqlens_q,
    ...
    Return:
        ...
        softmax_lse [optional, if return_attn_probs=True]: (nheads, total_q_seqlen). The
            ...
            normalization factor).
    """
    return FlashAttnF3B2VarlenFunc.apply(
        q,
        k,
        v,
        cu_seqlens_q,
